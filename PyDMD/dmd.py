...
class DMD(DMDBase):
	...
	# @property
	# def recon(self):
	# 	data = self.recon_fn_data(self, self.modes, self._eigs)
	# 	return data

	def recon_fn_data(self, s_modes, s_eig):
		"""
		numpy.ndarray: DMD reconstructed data.
		"""
		# soft the modes in descending order
		omega = old_div(np.log(s_eig), self.original_time['dt'])
		vander = np.exp(np.multiply(*np.meshgrid(omega, self.dmd_timesteps[0:-1])))
		b = np.linalg.lstsq(s_modes, self._X[:, 0])[0]
		return s_modes.dot((vander * b).T)
