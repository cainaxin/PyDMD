	...
	def fit(self, X, Y=None):
		...
		while data_queue:
			...
			try:
				...
				rho = old_div(float(self.max_cycles), n_samples) # max sampling frequency
			...
			if current_level < 2**(self.max_level - 1): # number of bins in current level
				current_level += 1
